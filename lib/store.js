...
const getExpires = (minutes = null) => {
    const time = minutes || ONE_MINUTE * 20
        ...
        case 'CONTINUE_REQUEST': {
            return {
                ...
                form: {
                    ...action.form,
                    RI_CB:
                        action.form.RI_CB.length === 1
                            ? action.form.RI_CB[0]
                            : action.form.RI_CB,
                    RI_MR_OPT: action.form.RI_MR_OPT[0],
                    RI_MR_FR_CB:
                        action.form.RI_MR_FR_CB.length === 1
                            ? action.form.RI_MR_FR_CB[0]
                            : action.form.RI_MR_FR_CB,
                    YI_PHC: action.form.YI_PN,
                    YI_EMC: action.form.YI_EM,
                    TRKNUM: action.trackingNumbers[0].TrackingNumberID,
                },
                authForm: null,
...
export const StoreProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        window.localStorage['vrc_healthcare_portal'] = JSON.stringify({
            ...
            continuedRequest: state.continuedRequest,
        })
    }, [state])
