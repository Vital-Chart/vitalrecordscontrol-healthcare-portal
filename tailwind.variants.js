export default {
    grids: {},
    texts: {
        intro: 'text-gray-dark font-light leading-relaxed',
    },
    headings: {
        DEFAULT: 'font-bold',
        display: 'text-display leading-none',
        h1: 'text-h1 leading-none',
        h2: 'text-h2 leading-tight',
        h3: 'text-h3 leading-tight',
        h4: 'text-h4 leading-tight',
        h5: 'text-h5',
        h6: 'text-h6',
    },
    links: {
        compact: 'uppercase text-sm tracking-wide',
    },
    buttons: {
        DEFAULT:
            'inline-flex items-center justify-center px-6 py-3 rounded transition-colors',
        filled:
            'text-white font-bold bg-primary hover:bg-tertiary hover:text-black focus:border-gray-dark',
        filledSecondary:
            'text-black font-bold bg-secondary hover:bg-gray hover:text-white focus:border-gray-dark',
        filledReverse:
            'text-primary font-bold bg-white hover:bg-tertiary hover:text-black focus:border-gray-dark',
        secondaryReverse:
            'text-secondary-dark font-bold bg-white hover:bg-secondary-lightest hover:text-black focus:border-gray-dark',
        outline:
            'text-primary font-bold bg-transparent ring-2 hover:bg-primary hover:text-white focus:bg-gray-lighter focus:border-gray-lighter',
    },
    images: {},
}
